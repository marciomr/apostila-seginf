...
Este modo de operação não garante a segurança contra ataques {\em ciphertext only} porque blocos iguais são criptografados de maneira igual (Figura \ref{fig:ecb-exemplo}\footnote{Imagem tirada do verbete {\em Modo de Operação (criptografia)} da Wikipédia (\url{https://pt.wikipedia.org})}).
...
\node (ml) at (6,0) {$m_l$};
\node (cl) at (6,-3) {$c_l$};
\draw[->] (m0) -> node[left]{$p_k$} (c0);
\draw[->] (m1) -> node[left]{$p_k$} (c1);
\draw[->] (ml) -> node[left]{$p_k$} (cl);
\end{tikzpicture}
\end{center}
...
O que estamos buscando é um sistema que seja seguro contra CPA.
...
\begin{eqnarray*}
  ...
  c_i & := & p_k(c_{i-1} \xor m_i) \textrm{ para } i = 1, \dots, l
\end{eqnarray*}
...
\begin{eqnarray*}
  m_{i} & := & p_k^{-1}(c_{i+1}) \xor c_{i} \textrm{ para } i = 0, \dots, l-1
\end{eqnarray*}
...
\begin{eqnarray*}
  c_0 & := & IV \leftarrow \{0,1\}^n \\
  c_i & := & m_i \xor f_k(IV + i) \textrm{ para } i = 1, \dots, l
\end{eqnarray*} 
...
\begin{eqnarray*}
  m_i & := & c_i \xor f_k(c_0 + i) \textrm{ para } i = 1, \dots, l
\end{eqnarray*} 
